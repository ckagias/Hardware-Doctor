...
pub fn keyboard_snapshot(state: &KeyboardState) -> KeyboardSnapshot {
...
pub fn key_down(state: &KeyboardState, code: String) {
...
pub fn key_up(state: &KeyboardState, code: String) {
    if !state.all_keys.contains(code.as_str()) {
        return;
    }
...
pub fn reset_tested(state: &KeyboardState) {
