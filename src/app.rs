...
impl eframe::App for TroubleApp {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        // Keeps the mic level meter / recording-done polling live every frame instead of only
        // on input events
        ...
        egui::SidePanel::left("sidebar").show(ctx, |ui| {
            for module in Module::ALL {
                ...
                if ui.selectable_label(selected, module.label()).clicked() && !selected {
                    // mirrors the old page-unmount cleanup: leaving the mic tab should stop
                    // the live monitor stream rather than let it keep running in the background
